...
class WalkAction
    def should_exec?(environment, status)
        @dir = environment.captives == 2 ? :forward : :backward
...
class RangedAttackAction
    def should_exec?(environment, status)
        ...
        @dir = possible_dirs.include?(:backward) ? :backward : possible_dirs.find {|dir| environment.unobstructed? ({dir: dir, dist: enemy_sensor.subject_min_dist(dir)})}
        !@dir.nil? && !enemy_sensor.subject?({dist: 0}) && environment.enemies != 2
    end
...
class RestAction
    ...
    def should_exec?(environment, status)
        status.health < @min_health && !status.took_dmg? && environment.enemies > 1
    end
